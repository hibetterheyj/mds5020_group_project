...
# XGBoost 随机搜索参数分布
xgb_param_dist = {
    'n_estimators': randint(10, 300),    # 决策树数量
    'max_depth': randint(3, 20),         # 树深度
    'learning_rate': uniform(0.01, 0.3),           # 初始学习率（等价于learning_rate）
    'subsample': uniform(0.6, 0.4),      # 样本采样比例
    'colsample_bytree': uniform(0.6, 0.4),# 特征采样比例
    'scale_pos_weight': randint(1, 10),  # 类别不平衡权重
    'lr_decay': uniform(0.9, 0.1),       # 学习率衰减系数（0.9~1.0，越接近1衰减越慢）
    'decay_step': randint(1, 50)         # 每1~50轮衰减一次（默认1，即每轮衰减）
}
...
print(f"XGBoost Best F1 Score: {random_search.best_score_:.4f}")
...
pipeline_objects = joblib.load('optimized_xgboost_final.pkl')
...
test_df[['prediction', 'probability_yes']].to_csv('predictions_xgboost_final.csv', index=False)
print("XGBoost prediction completed! Results saved to predictions_xgboost_final.csv")
